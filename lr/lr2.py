...
onehot特征：
    ...
    小时数，展示页码编号
    店铺好评率/店铺服务评分/店铺物流评分/店铺描述评分，点评星级，店铺评论数，店铺id

    上下文预测类目个数，上下文预测类目与商品类目交集个数，上下文中该类目的预测属性个数，上下文属性与商品属性交集个数，
    用户距离上次点击时长（秒），用户距离下次点击时长，用户过去一小时点击量，用户未来一小时点击量，用户前一天点击量，用户前一天转化率，用户历史转化率，用户前后两次点击时长差值，用户前后两小时点击次数差值
    用户年龄与类目年龄差值
    商品历史转化率（当天以前），商品属性个数，商品前一天交易量，商品前一天转化率，商品前一天交易量与同类差值，商品前一天转化率与同类差值
    用户年龄与商品平均用户年龄差值
    ...
    店铺商品数目
    店铺评论数与同类之差，店铺星级与同类之差
    品牌历史转化率，品牌前一天转化率，
    品牌平均收藏与同类差值，品牌平均价格与同类差值，品牌平均广告与同类差值，品牌平均销量与同类差值，
    用户年龄与品牌平均年龄之差，
    店铺该品牌商品占有率，店铺品牌数与商品数比值

    用户在该类目的历史转化率，用户距离下次点击该类目时长，用户前后两次点击该类目时间之差，用户在该类目前后两小时点击数差值，用户在该类目平均价格，商品价格与用户在该类目平均价格之差
...
前期处理：对数值特征进行归一化缩放处理，对onehot特征的特征值进行分区，并合并数目过少的分区
结果： A榜（0.14482）
...
if __name__ == '__main__':
    ...
    strFea = [
        ...
        'shop_star_level','shop_review_num_level','shop_review_positive_rate_level','shop_score_service_level','shop_score_delivery_level','shop_score_description_level','shop_id',
